      ...
      <div className="bg-white rounded-xl p-6 shadow-sm mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {hasPermission('manage_projects') && (
            <Link href="/admin/projects" className="flex items-center gap-3 p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
              <FolderKanban className="w-5 h-5 text-orange-500" />
              <span className="text-slate-700">Quản lý dự án</span>
            </Link>
          )}
          {hasPermission('manage_products') && (
            <Link href="/admin/products" className="flex items-center gap-3 p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
              <Box className="w-5 h-5 text-orange-500" />
              <span className="text-slate-700">Quản lý sản phẩm</span>
            </Link>
          )}
          {hasPermission('manage_services') && (
            <Link href="/admin/services" className="flex items-center gap-3 p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
              <Briefcase className="w-5 h-5 text-orange-500" />
              <span className="text-slate-700">Quản lý dịch vụ</span>
            </Link>
          )}
          {hasPermission('manage_banners') && (
            <Link href="/admin/banners" className="flex items-center gap-3 p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
              <Layout className="w-5 h-5 text-orange-500" />
              <span className="text-slate-700">Quản lý banner</span>
            </Link>
          )}
          {hasPermission('manage_articles') && (
            <Link href="/admin/articles" className="flex items-center gap-3 p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
              <FileText className="w-5 h-5 text-orange-500" />
              <span className="text-slate-700">Quản lý bài viết</span>
            </Link>
          )}
          {hasPermission('manage_reviews') && (
            <Link href="/admin/reviews" className="flex items-center gap-3 p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
              <Star className="w-5 h-5 text-orange-500" />
              <span className="text-slate-700">Quản lý đánh giá</span>
            </Link>
          )}
          {hasPermission('manage_images') && (
            <Link href="/admin/images" className="flex items-center gap-3 p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
      ...
      {hasPermission('manage_projects') && (
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/admin/projects" className="text-orange-500 hover:text-orange-600 text-sm font-medium">
              Xem tất cả
            </Link>
          </div>
          {loading ? (
