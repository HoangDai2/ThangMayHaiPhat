...
export default function AdminBanners() {
  ...
  const fetchBanners = async () => {
    setLoading(true);
    try {
      const response = await api.get('/banners?sort_by=sort_order&sort_dir=asc');
  ...
  const toggleActive = async (b: Banner) => {
    try {
      await api.put(`/banners/${b.id}`, { is_active: !b.is_active });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && form.id) {
        await api.put(`/banners/${form.id}`, dataToSave);
      } else {
